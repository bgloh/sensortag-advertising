...
// DATA ORDER = [AccX_L AccX_H AccY_L AccY_H AccZ_L AccZ_H]

+(float) calcXValue:(NSData *)data {
    char scratchVal[6];
    char LowByteAccX, HighByteAccX;
    float accX;
    [data getBytes:&scratchVal length:6];
    LowByteAccX = scratchVal[0];
    HighByteAccX = scratchVal[0];
    accX = (HighByteAccX ) * 1.0 / 256 * KXTJ9_RANGE;
    return accX;
}
+(float) calcYValue:(NSData *)data {
    //Orientation of sensor on board means we need to swap Y (multiplying with -1)
    ...
    char scratchVal[6];
    char LowByteAccY, HighByteAccY;
    float accY;
    [data getBytes:&scratchVal length:6];
    LowByteAccY = scratchVal[2];
    HighByteAccY = scratchVal[3];
    accY = ((HighByteAccY << 4) + (LowByteAccY & 0x0F)) * 1.0 / 1024 * KXTJ9_RANGE;
    return accY;
}
+(float) calcZValue:(NSData *)data {
    char scratchVal[6];
    int16_t rawData[3];
    char LowByteAccZ, HighByteAccZ;
    int16_t accZCount ;
    float accZ;
    [data getBytes:&scratchVal length:6];
//     [data getBytes:&rawData length:6];
//    LowByteAccZ = scratchVal[4];
//    HighByteAccZ = scratchVal[5];
   // accZCount = ((HighByteAccZ  * 1.0) + LowByteAccZ * 0.0 );
   // accZCount = ((scratchVal[5] &  0xf0)>>4) * 1.0;
  // accZCount = (((scratchVal[4] & 0x08)) * (-1) + (scratchVal[4] & 0x07))*256 + scratchVal[5];
   //  accZCount = scratchVal[5] & 0x0F;
   // accZCount = rawData[2] & 0xFFF;
  //  accZ = (accZCount * 1.0) / 1024 * 1.0;
 //   accZCount = (scratchVal[4] & 0xff) | ((scratchVal[5] << 8) & 0x0F00);
  //  accZCount = (scratchVal[2] & 0x00ff) * 1.0;
    accZ = scratchVal[0] * 1.0;
    return accZ;
