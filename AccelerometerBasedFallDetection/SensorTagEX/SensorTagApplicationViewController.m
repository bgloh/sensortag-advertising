...
-(id) initWithStyle:(UITableViewStyle)style andSensorTag:(BLEDevice *)andSensorTag {
    ...
    if (self) {
        ...
        if (!self.acc) {
            ...
            self.acc.accIcon.image = [UIImage imageNamed:@"accelerometer.png"];
...
-(void) configureSensorTag {
    ...
    if ([self sensorEnabled:@"Accelerometer active"]) {
        ...
        uint8_t rangeData = 0x03;  // 8G: 01, 4G:02,2G:03
        [BLEUtility writeCharacteristic:self.d.p sCBUUID:sUUID cCBUUID:cUUID data:[NSData dataWithBytes:&rangeData length:1]];
    ...
    if ([self sensorEnabled:@"Gyroscope active"]) {
        ...
        CBUUID *pUUID =  [CBUUID UUIDWithString:[self.d.setupData valueForKey:@"Gyroscope period UUID"]];

        uint8_t data = 0x07; // all 3 axes enabled
        NSInteger period = [[self.d.setupData valueForKey:@"gyroscope period"] integerValue];
        uint8_t periodData = (uint8_t)(period / 10);
        [BLEUtility writeCharacteristic:self.d.p sCBUUID:sUUID cCBUUID:cUUID data:[NSData dataWithBytes:&data length:1]];
        // set gyroscope measurement period
        [BLEUtility writeCharacteristic:self.d.p sCBUUID:sUUID cCBUUID:pUUID data:[NSData dataWithBytes:&periodData length:1]];
    ...
    // connection service
    if ([self sensorEnabled:@"RequestConnection active"]) {
    CBUUID *sUUID = [CBUUID UUIDWithString:[self.d.setupData valueForKey:@"Connection service UUID"]];
    CBUUID *cUUID = [CBUUID UUIDWithString:[self.d.setupData valueForKey:@"RequestConnection UUID"]];

    uint64_t connectionParameter = 0x00140014001403EB;
   [BLEUtility writeCharacteristic:self.d.p sCBUUID:sUUID cCBUUID:cUUID data:[NSData dataWithBytes:&connectionParameter length:8]];
    }
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:[self.d.setupData valueForKey:@"Accelerometer data UUID"]]]) {
        ...
        float Rotx,Rotz;  // added code for tilt angle display
        ...
        self.acc.accValueX.textColor = [UIColor blueColor];
        self.acc.accValueY.textColor = [UIColor greenColor];
        self.acc.accValueZ.textColor = [UIColor redColor];
        ...
        // TILT ANGLES DISPLAY
        self.tiltAngles = [self calcTiltAngleFromAccelerationX:x andAccelerationY:y andAccelerationZ:z];
        Rotz = *self.tiltAngles;
        Rotx = *(self.tiltAngles+1);
        self.acc.xTiltAngle.text =[NSString stringWithFormat:@"PITCH: %0.1fDEG",Rotz];
        self.acc.yTiltAngle.text =[NSString stringWithFormat:@"ROLL: %0.1fDEG",Rotx];
        self.acc.xTiltAngle.textColor =[UIColor redColor];
        self.acc.yTiltAngle.textColor =[UIColor blueColor];
       // NSLog(@"x:%1.2f,y:%1.2f,z:%1.2f",x,y,z);
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:[self.d.setupData valueForKey:@"Gyroscope data UUID"]]]) {
        ...
        self.gyro.accValueX.textColor = [UIColor blueColor];
        self.gyro.accValueY.textColor = [UIColor greenColor];
        self.gyro.accValueZ.textColor = [UIColor redColor];
...
// calculate tilt angles from acceleration in x,y, and z
    -(float *) calcTiltAngleFromAccelerationX:(float)accX andAccelerationY:(float)accY andAccelerationZ:(float)accZ {
// float ax;
// float ay;
// float az;
 double phi;
 double theta;
 float  angle[2];
 // TRANSFORM SENOSORTAG ACCELERATIONS INTO TILT ANGLE FORMULAR
/* ax = accX;      // acceleration in x-direction
 ay = -accZ;     // acceleration in y-direction
 az = accY;      // acceleration in z-direction
 phi = atan(ax/sqrt(pow(ay,2)+pow(az,2)))*180.0/3.14f; // x-axis tilt
 ro = atan(ay/sqrt(pow(ax,2)+ pow(az,2)))*180.0/3.14f;  // y-axis tilt
       if (accY < 0 && accZ > 0)
            ro = -(180+ro);  // calculate ro from -180 ~ +90
        else*/
        
        
// noise removal. Withiout this, theta defined as atan(accX/accY) produces an erroneous result stemming from the ratio, accX/accY.
if (fabsf(accX) <0.05)
    accX = 0.00001;
if (fabsf(accZ) <0.05)
    accZ = 0.00001;
        
// TILT ANGLES BASED ON SENSORTAG COORDINATES
//phi = -atan(accX/sqrt(pow(accY,2)+pow(accZ,2)))*180.0/3.14f; // Rotation about SENSORTAG Z-AXIS
theta = -atan(accX/accY)*180.0/3.14f; // Rotation about SENSORTAG Z-AXIS (PITCH)
phi = -atan(accZ/sqrt(pow(accX,2)+ pow(accY,2)))*180.0/3.14f;  // Rotation about SENSORTAG X-AXIS(ROLL)
        
        if (accY < 0 && accZ > 0)
            phi = -(180+phi);  // calculate ro from -180 ~ +90
...
-(IBAction)sendMail:(id)sender {
  ...
  /*  [sensorData appendString:@"Timestamp,Ambient Temperature,IR Temperature,Accelerometer X-Axis,Accelerometer Y-Axis,Accelerometer Z-Axis,Barometric Pressure,Relative Humidity,Gyroscope X,Gyroscope Y,Gyroscope Z,Magnetometer X, Magnetometer Y, Magnetometer Z\n"];*/
    [sensorData appendString:@"Timestamp,Accelerometer X-Axis,Accelerometer Y-Axis,Accelerometer Z-Axis,Gyroscope X,Gyroscope Y,Gyroscope Z \n"];
    for (int ii=0; ii < self.vals.count; ii++) {
     ...
     /*   [sensorData appendFormat:@"%@,%0.1f,%0.1f,%0.2f,%0.2f,%0.2f,%0.0f,%0.1f,%0.1f,%0.1f,%0.1f,%0.1f,%0.1f,%0.1f\n",s.timeStamp,s.tAmb,s.tIR,s.accX,s.accY,s.accZ,s.press,s.humidity,s.gyroX,s.gyroY,s.gyroZ,s.magX,s.magY,s.magZ];*/
        [sensorData appendFormat:@"%@,%0.2f,%0.2f,%0.2f,%0.1f,%0.1f,%0.1f\n",s.timeStamp,s.accX,s.accY,s.accZ,s.gyroX,s.gyroY,s.gyroZ];
    }
...
-(void)UrlPost{
    NSMutableURLRequest *request = [NSMutableURLRequest
                                    requestWithURL:[NSURL URLWithString:@"https://script.google.com/macros/s/AKfycbzH5v-xbv7JKFtlrcITZeUZKwEOmH01dZ9rl7i5MKw962AkBGg/exec"] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:30.0f];
    
    //build an info object and convert to json
    
//    NSString *post = [NSString stringWithFormat:@"A:1,'B:3"];
//    NSData *postData = [post dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
 /*   NSMutableDictionary *post = [[NSMutableDictionary alloc]init];
   [post setValue :@"Brian" forKey:@"A"];
   [post setValue :@"AweSome" forKey:@"B"];
    NSLog(@"post****************************:%@",post);*/
  /*NSDictionary* post = [NSDictionary dictionaryWithObjectsAndKeys:
    ...
    valB = 99;
    valC = 90;
    NSData *postData = [[[NSString alloc] initWithFormat:@"A=%d&B=%d&C=%d", valA, valB,valC] dataUsingEncoding:NSUTF8StringEncoding];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    NSString *responeString = [[NSString alloc] initWithData:self.responseData
                                                    encoding:NSUTF8StringEncoding];
    NSLog(@"responseString:%@",responeString);
    // Assume lowercase
