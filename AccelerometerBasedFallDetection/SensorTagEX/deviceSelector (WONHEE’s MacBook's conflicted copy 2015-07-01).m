...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        self.title = @"Accelerometer-based Fall Alarm";
    }
    return self;
}
...
-(void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
  ...
  CBUUID *sUUIDGyro = [CBUUID UUIDWithString:[self.serviceArray valueForKey:@"Gyroscope service UUID"]];
    ...
    [NSTimer scheduledTimerWithTimeInterval:0.1f target:peripheral selector:@selector(MyreadRSSI:) userInfo:nil repeats:YES];
...
-(NSMutableDictionary *) makeSensorTagConfiguration {
    ...
    [d setValue:@"0" forKey:@"RequestConnection active"];
